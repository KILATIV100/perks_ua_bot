...
### Крок 6: Налаштування власного домену (Custom Domain)

**Важливо:** Якщо ви хочете використовувати власний домен (наприклад, `perkup.com.ua`), потрібно додати його в Railway:

1. Відкрийте Railway Dashboard → Ваш проєкт → **Client** сервіс
2. Перейдіть до **Settings** → **Networking** → **Custom Domains**
3. Натисніть **"Add Custom Domain"**
4. Введіть ваш домен: `perkup.com.ua`
5. Railway покаже DNS налаштування (зазвичай CNAME запис)
6. Налаштуйте DNS у вашого реєстратора:
   - **Тип:** CNAME
   - **Ім'я:** @ (або залиште порожнім для кореневого домену)
   - **Значення:** `<значення-з-railway>.railway.app`
7. Зачекайте 5-15 хвилин для випуску SSL сертифікату

**Примітка:** Railway автоматично випускає та оновлює SSL сертифікати через Let's Encrypt. Без налаштування Custom Domain браузер покаже помилку `NET::ERR_CERT_COMMON_NAME_INVALID`.
